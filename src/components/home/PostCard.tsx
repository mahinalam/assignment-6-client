...
const PostCard = ({ item }: any) => {
  ...
  const { _id, images, votes, user, upVotes, downVotes } = item;
  ...
  // if()
  const [isVoteAdded, setIsVoteAdded] = useState(false);
  ...
  const myOwnComments =
    ...
    allPostComments.filter((item) => item?.user?._id === currentUserInfo?._id);

  console.log("myOwnComments", myOwnComments);
  ...
  if (commentsDataLoading) return <p>Loading ...</p>;
  ...
  const handleUpVotes = (status: string) => {
    if (!isOwnUpVote && !isOwnDownVote) {
      ...
      console.log("likeStatusData", data);
    }
  };
      ...
      <div>
        <div className="flex items-center justify-between gap-4 mt-2">
          <div className="flex items-center gap-10 ">
            <svg
              className={`size-7 ${isOwnDownVote || isOwnUpVote ? (isOwnUpVote ? "text-blue-500 cursor-not-allowed" : "cursor-not-allowed") : "cursor-pointer"} `}
              fill="none"
              stroke="currentColor"
              strokeWidth="1.5"
              viewBox="0 0 24 24"
            ...
            <svg
              ...
              className={`size-7 ${isOwnDownVote || isOwnUpVote ? (isOwnDownVote ? "text-blue-500 cursor-not-allowed" : "cursor-not-allowed") : "cursor-pointer"} `}
            >
              <path
        ...
        <ModalContent>
          ...
          <ModalBody>
            <div className="grid md:grid-cols-2 grid-cols-1 gap-5">
              <div>
                {/* <Image alt=""  src={img} /> */}
                <img alt="" className="h-[80vh] w-full" src={images[0]} />
              </div>
              <div className="">
